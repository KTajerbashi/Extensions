...
public class DapperEntityChangeInterceptorItemRepository : IEntityChangeInterceptorItemRepository
{
    ...
    private string InsertEntityChangeInterceptorItemCommand = "INSERT INTO [{0}].[{1}]([Id],[ContextName],[EntityType],[EntityId],[UserId],[IP],[TransactionId],[DateOfOccurrence],[ChangeType]) VALUES (@Id,@ContextName,@EntityType,@EntityId,@UserId,@IP,@TransactionId,@DateOfOccurrence,@ChangeType) ";
    private string InsertPropertyChangeLogItemCommand = "INSERT INTO [{0}].[{1}]([Id],[ChangeInterceptorItemId],[PropertyName],[Value]) VALUES (@Id,@ChangeInterceptorItemId,@PropertyName,@Value)";
